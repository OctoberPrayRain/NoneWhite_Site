...
一个资源分享站，用户可以浏览资源详情页、点赞评论、获取下载链接与资源文件。

本地运行说明、常用页面与功能使用见根目录 `README.md`。这份 `DEVELOP.md` 文档记录开发计划与 API 细节。
...
以下成功路径需要先启动 PostgreSQL、应用 `server/migrations/20260605000000_create_users.sql`，并启动后端服务。示例只记录占位 token，不能把真实 JWT 写入公开文档或协作日志。
...
以下示例需要先应用 Phase 2/3/4 migrations，并启动后端。需要登录的接口只记录占位 token，不能把真实 JWT 写入公开文档或协作日志。
...
以下示例需要先应用 Phase 2/3/4/5 migrations，并启动后端。管理员接口必须使用 `users.role='admin'` 的登录 token；示例只记录占位 token，不能把真实 JWT、真实网盘链接或生产密码写入公开文档或协作日志。
