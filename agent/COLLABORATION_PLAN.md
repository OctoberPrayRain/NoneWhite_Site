...
- 数据库：PostgreSQL 17，当前只有 `docker-compose.yml` 和环境变量模板。
- 已实现：Phase 1 项目骨架、前端首页、`/test-api` 页面、后端 `GET /api/test`。
- 未实现：数据库 schema/migration、用户系统、认证鉴权、游戏浏览、互动功能、管理后台、搜索和部署。
...
头像上传必须先确认存储策略。未确认前，不能开始后端正式实现，只能做前端占位。
...
| `JWT_EXPIRES_IN_SECONDS` | `server/.env.example` 和根 `.env.example` | JWT 过期时间 | 数字，单位秒 |
| `UPLOAD_DIR` | 如实现头像上传 | 上传文件目录 | 必须说明是否进入 Git 忽略 |
| `MAX_AVATAR_SIZE_BYTES` | 如实现头像上传 | 头像最大大小 | 数字，单位 byte |
...
| `avatar` | 文件类型、大小、存储策略确认后实现 | 上传前提示类型和大小 |
...
- `uploadAvatar` 只在后端上传契约确认后实现；未确认时保留 TODO 文档，不写假接口。
